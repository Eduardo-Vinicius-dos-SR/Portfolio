export default function ContactItem({ href, title, description, children }: { href: string, title: string, description: string, children: React.ReactNode }) {
    return (
        <li>
            <a href={href} target="_blank" rel="noopener noreferrer" aria-label={`Abrir ${title}`} className="flex items-center p-10 border-t gap-20 pointer duration-300ms ease-in transition-all hover:text-[var(--text-h)]">
